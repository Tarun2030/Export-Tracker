...
import { getCustomers, createOrder, getOrder, updateOrder } from '@/lib/data-service';
...
export default function OrderForm({ orderId }: OrderFormProps) {
  ...
  useEffect(() => {
    ...
    async function fetchOrderForEdit() {
      ...
      try {
        ...
        setForm({
          ...
          exchange_rate: String(order.exchange_rate ?? '84.00'),
          inr_value: String(order.inr_value ?? ''),
          delivery_terms: order.delivery_terms || 'FOB',
          payment_terms: order.payment_terms || '30 days LC',
          lc_number: order.lc_number || '',
          lc_date: order.lc_date || '',
          lc_expiry_date: order.lc_expiry_date || '',
          lc_amount: String(order.lc_amount ?? ''),
          lc_bank: order.lc_bank || '',
          destination_port: order.destination_port || '',
          origin_port: order.origin_port || 'INMUN',
          shipping_bill_number: order.shipping_bill_number || '',
          shipping_bill_date: order.shipping_bill_date || '',
          gst_invoice_number: order.gst_invoice_number || '',
          gst_invoice_date: order.gst_invoice_date || '',
          gst_amount: String(order.gst_amount ?? '0'),
          igst_amount: String(order.igst_amount ?? '0'),
          rodtep_claim: String(order.rodtep_claim ?? '0'),
          rodtep_status: order.rodtep_status || 'pending',
          drawback_amount: String(order.drawback_amount ?? '0'),
          drawback_status: order.drawback_status || 'pending',
    ...
    try {
      ...
      toast({
        ...
        description: isEditMode
          ? `Order ${form.order_number} has been updated successfully`
          : `Order ${form.order_number} has been created successfully`,
      });
      router.push('/orders/list');
    } catch (error) {
      toast({
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{isEditMode ? 'Edit Order' : 'Add New Order'}</h1>
          <p className="text-sm text-slate-500 mt-1">
            {isEditMode ? 'Update existing export order' : 'Create a new export order'}
          </p>
        </div>
      </div>
